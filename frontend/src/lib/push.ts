...
export async function ensurePushSubscription(endpoint: string): Promise<boolean> {
  ...
  try {
    ...
    // Network failure, an unsupported/rejecting browser API, etc. — the
    // import still works via in-app realtime updates and toasts, so push
    // registration failures must never surface as an unhandled rejection.
    return false;
  }
}
