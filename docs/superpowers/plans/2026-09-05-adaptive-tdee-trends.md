...
- **Budget-resolution logic duplication.** The `trendsBudget` / `formulaTDEE` / `formulaInput` calculation in `backend/internal/routes/trends.go` duplicates `userBudget` in `backend/internal/routes/summary.go`; a future change must be made in both places, with nothing enforcing it. Fold into a shared helper the next time either changes.
- **Per-metric recorded flag.** The payload cannot distinguish "recorded 0 water/steps" from "recorded nothing", so `MetricCard` treats a genuine zero as unrecorded. The fix is a per-metric recorded flag on `dayJSON`, mirroring the existing `logged` flag for food entries.
- **User-scoping coverage.** User-scoping is pinned by test only for `diary_entries`; the `weights`, `daily_metrics` and `meal_slots` queries in the trends endpoint are correct by inspection but unguarded against a future edit dropping their `user = {:uid}` bind.
- **Empty-range test coverage.** No screen-level test covers `BalanceCard` or `IntakeCard` rendering for a fully-unlogged range, the way `WeightCard`'s and `MealsCard`'s empty states are pinned.
- **MealsCard unreachable branch.** `MealsCard`'s `|| rows.length === 0` condition is now unreachable via the screen, because `mealRows` returns `[]` only when `loggedDays === 0`, and the `meals` card's `minDays: 7` gate intercepts that state first. The branch is defensive and worth keeping to guard against direct component use or future gate changes, but is untested at the screen level. Could be dropped, or moved to a component-level test, when someone next touches that file.
- **Optimistic toggle rollback.** `saveCards` in `Trends.tsx` does not roll back its optimistic toggle when the profile write fails, and no test simulates that failure.
